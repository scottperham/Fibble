import { ClockIcon, ShareIcon, PlayIcon } from '@heroicons/react/outline'
...
import { getGameDate, getNextGameDate } from '../../lib/words'
      ...
      {(isGameLost || isGameWon) && (
        <div className="mt-5 columns-2 items-center items-stretch justify-center text-center dark:text-white sm:mt-6">
          <div className="inline-block w-full text-left">
            {(isDaily && (!ENABLE_ARCHIVED_GAMES || isLatestGame)) && (
              <div>
                <h5>{NEW_WORD_TEXT}</h5>
                <Countdown
                  className="text-lg font-medium text-gray-900 dark:text-gray-100"
                  date={getNextGameDate(getGameDate()).valueOf()}
                  daysInHours={true}
                />
              </div>
            )}
